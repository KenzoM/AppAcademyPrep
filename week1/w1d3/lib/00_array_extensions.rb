...
class Array
  def sum
    self.inject(0){|accum,sum| accum +  sum }
  end
end
...
class Array
  def my_uniq
    collection = []
    each do |x|
      collection << x if !collection.include?(x)
    end
    collection
  end
end
...
class Array
  def median
    ...
    return (self[size/2] + self[(size/2)- 1]) / 2.0
  end
end
...
class Array
  def my_transpose
    template = self.map{|x,y| x= nil, y = nil} #Create a template
    self.size.times do |x|
      self.size.times do |y|
        template[x][y] = self[y][x]
      end
    end
    template
  end
